...
<head>
    ...
    <style>
        ...
        @keyframes pulse-orange { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.05); } }
        .btn-urgente { animation: pulse-orange 2s infinite; }
    </style>
</head>
<body class="bg-baby pb-24 font-sans overflow-x-hidden">
    ...
    <div id="setup-screen" class="hidden fixed inset-0 bg-white z-50 p-8 flex flex-col justify-center text-center">
        <div id="step-1" class="setup-step space-y-4">
            ...
            <p class="text-[10px] text-slate-400 mt-4 italic">Podrás añadir más bebés desde Ajustes.</p>
        </div>
        ...
        <div class="bg-white w-72 h-full p-6 shadow-xl space-y-6 overflow-y-auto">
            ...
            <div class="space-y-4">
                <div class="p-4 bg-blue-50 rounded-2xl">
                    <p class="text-[10px] font-bold text-blue-700 uppercase mb-2">Horas entre tomas</p>
                ...
                <button onclick="alert('Próximamente: Podrás invitar a tu pareja o familiares para registrar datos en tiempo real.')" class="w-full flex items-center gap-3 p-4 bg-green-50 rounded-xl font-bold text-green-700 border border-green-100">
                    <i class="fa-solid fa-user-group"></i> Añadir Cuidador
                </button>

                <button onclick="alert('Funcionalidad multi-bebé en desarrollo')" class="w-full flex items-center gap-3 p-4 bg-slate-50 rounded-xl font-bold text-slate-500">
                ...
                <button onclick="borrarTodo()" class="w-full flex items-center gap-3 p-4 bg-red-50 rounded-xl font-bold text-red-600 text-xs">
                    <i class="fa-solid fa-trash"></i> Eliminar datos locales
                </button>
            </div>
        </div>
    </div>
    ...
    <main class="p-4 max-w-md mx-auto space-y-6">
        ...
        <div class="flex gap-2">
            <button onclick="addCompraPrompt()" class="flex-1 bg-orange-500 text-white p-4 rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all">
                <i class="fa-solid fa-cart-plus text-lg"></i> ¿FALTA ALGO?
            </button>
            <div id="compra-alert" class="hidden flex-1 bg-white border-2 border-orange-500 text-orange-600 p-4 rounded-2xl font-black text-[10px] flex items-center justify-center gap-2 btn-urgente" onclick="showTab('salud')">
                <i class="fa-solid fa-triangle-exclamation"></i> VER COMPRA
            </div>
        </div>
        ...
        <section id="sec-diario" class="space-y-4">
            <div class="grid grid-cols-2 gap-3">
                <button onclick="openPechoModal()" class="col-span-2 bg-pink-50 p-6 rounded-2xl text-pink-700 font-black shadow-sm flex items-center justify-center gap-3">
        ...
        <section id="sec-calendario" class="hidden space-y-4">
            ...
            <div class="bg-indigo-50 p-4 rounded-3xl border border-indigo-100">
                <h5 class="text-[10px] font-black text-indigo-800 uppercase mb-2">Sugerencias Vacunación</h5>
            ...
            <div id="day-detail" class="bg-blue-50 p-4 rounded-3xl border border-blue-100 hidden">
                ...
                <button onclick="openEventModal()" class="w-full mt-3 bg-blue-600 text-white p-3 rounded-xl font-bold text-xs">+ Añadir Evento</button>
            </div>
        </section>
    ...
    <div id="pecho-modal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-end">
        <div class="bg-white w-full p-6 rounded-t-3xl space-y-4">
            ...
            <div class="grid grid-cols-3 gap-2">
                <button onclick="registrarPecho('Izquierdo')" class="bg-blue-50 p-4 rounded-xl font-bold text-blue-700">Izquierdo</button>
                <button onclick="registrarPecho('Ambos')" class="bg-purple-50 p-4 rounded-xl font-bold text-purple-700">Ambos</button>
                <button onclick="registrarPecho('Derecho')" class="bg-blue-50 p-4 rounded-xl font-bold text-blue-700">Derecho</button>
            </div>
            <button onclick="closePechoModal()" class="w-full p-3 text-slate-400 font-bold text-xs uppercase">Cancelar</button>
        </div>
    </div>
    ...
    <div id="event-modal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-6">
        <div class="bg-white w-full rounded-3xl p-6 space-y-4">
            ...
            <input id="ev-titulo" type="text" placeholder="Título" class="w-full p-4 bg-slate-50 rounded-xl border-none outline-none">
            ...
            <button onclick="closeEventModal()" class="w-full text-slate-400 font-bold text-xs uppercase text-center mt-2">Cerrar</button>
        </div>
    </div>

    <script>
        // Mantenemos la lógica anterior (init, renderHeader, timers, calendar, vaccines, etc.)
        // Añadimos/Actualizamos funciones de Compra
        ...
        const VACUNAS_2026 = [
            { m: 0, n: "Hepatitis B" }, { m: 2, n: "Hexavalente, MenB, Rotavirus" }, 
            { m: 4, n: "Hexavalente, MenB, MenC, Rotavirus" }, { m: 11, n: "Refuerzo Hexavalente" }, 
            { m: 12, n: "Triple Vírica, MenACWY" }, { m: 15, n: "Varicela, Hep A" }
        ];

        function init() {
            bData = JSON.parse(localStorage.getItem('baby_pro_v3_data'));
            if(!bData) {
                ...
                renderHeader(); renderLog(); renderMedicina(); renderCompra(); renderVaccines();
        ...
        function addCompraPrompt() {
            const it = prompt("¿Qué falta comprar? (Pañales, Leche, Toallitas...)");
            if(it) { bData.compra.push(it); save(); renderCompra(); }
        }
        function renderCompra() {
            const hasItems = bData.compra.length > 0;
            document.getElementById('compra-alert').classList.toggle('hidden', !hasItems);
            document.getElementById('lista-compra').innerHTML = bData.compra.map((it, i) => `
                <div class="bg-white p-3 rounded-xl flex justify-between items-center text-xs border border-orange-100 shadow-sm">
                    <span class="font-bold text-slate-700">${it}</span>
                    <button onclick="borrarCompra(${i})" class="text-green-500"><i class="fa-solid fa-check"></i></button>
                </div>`).join('') || '<p class="text-[10px] text-center opacity-30 italic">Todo comprado</p>';
        }
        function borrarCompra(i) { bData.compra.splice(i,1); save(); renderCompra(); }

        // --- FUNCIONES RESTANTES (Compactas para el lanzamiento) ---
        function save() { localStorage.setItem('baby_pro_v3_data', JSON.stringify(bData)); }
        function getEdadTexto(f) { const d = Math.floor((new Date() - new Date(f)) / 86400000); return d < 30 ? d + " días" : Math.floor(d/30.44) + " meses"; }
        function renderHeader() {
            ...
            document.getElementById('top-peso-nac').innerText = nac.p + 'kg'; document.getElementById('top-talla-nac').innerText = nac.m + 'cm';
            document.getElementById('top-peso-act').innerText = act.p + 'kg'; document.getElementById('top-talla-act').innerText = act.m + 'cm';
            document.getElementById('top-fecha-act').innerText = act.fecha.split('-').reverse().slice(0,2).join('/');
            if(bData.lastBib) { document.getElementById('btn-last-bib').innerText = "🍼 Repetir " + bData.lastBib + "ml"; document.getElementById('btn-last-bib').classList.remove('hidden'); }
        }
        function updateAllTimers() {
            const container = document.getElementById('timers-container'); let html = ""; const ahora = Date.now();
            ...
            bData.medicinas.forEach(m => { if(m.ultimaToma) { const px = m.ultimaToma + (m.cada * 3600000); html += renderTimerRow("💊 " + m.nombre, px - ahora); if(ahora >= px && !notifiedMeds.has(m.nombre + px)) { document.getElementById('alert-sound').play(); notifiedMeds.add(m.nombre + px); } } });
            const hoyIso = new Date().toISOString().split('T')[0];
            if(bData.eventos[hoyIso]) bData.eventos[hoyIso].forEach(e => { const [h, m] = e.hora.split(':'); const eMs = new Date().setHours(h, m, 0, 0); if(eMs > ahora) html += renderTimerRow("📅 " + e.titulo, eMs - ahora); });
            container.innerHTML = html || '<p class="text-[10px] text-center opacity-30 italic">Sin tareas pendientes</p>';
        }
        function renderTimerRow(label, diff) {
            const s = Math.floor(diff/1000);
            if(s <= 0) return `<div class="bg-red-500 text-white p-3 rounded-xl flex justify-between font-black text-[10px]"><span>${label}</span><span>YA!</span></div>`;
            const time = Math.floor(s/3600) + ":" + String(Math.floor((s%3600)/60)).padStart(2,'0') + ":" + String(s%60).padStart(2,'0');
            return `<div class="bg-white p-3 rounded-xl border flex justify-between text-[10px] font-bold text-slate-500"><span>${label}</span><span class="text-blue-600 font-mono">${time}</span></div>`;
        }
        function registrar(txt) {
            const iso = new Date().toISOString().split('T')[0], h = new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
            if(!bData.logs[iso]) bData.logs[iso] = []; bData.logs[iso].unshift({txt, h});
            if(txt.includes('Biberón') || txt.includes('Pecho')) localStorage.setItem('last_toma_time', Date.now());
            save(); renderLog();
        }
        function showTab(t) { ['diario','calendario','salud'].forEach(s => { document.getElementById('sec-'+s).classList.toggle('hidden', s!==t); document.getElementById('tab-'+s).classList.toggle('active-tab', s===t); }); if(t==='calendario') renderFullCalendar(); }
        function renderFullCalendar() {
            const grid = document.getElementById('days-container'); grid.innerHTML = ''; const y = viewDate.getFullYear(), m = viewDate.getMonth();
            ...
            for(let d=1; d<=days; d++) {
                ...
                grid.innerHTML += `<div onclick="selectCalendarDay('${ds}')" class="aspect-square flex flex-col items-center justify-center rounded-xl relative ${new Date().toISOString().split('T')[0] === ds?'bg-blue-600 text-white':'text-slate-700'} ${selectedDayStr===ds?'ring-2 ring-blue-300':''}">
                    <span class="text-xs font-bold">${d}</span>${hasEv ? '<div class="w-1 h-1 bg-orange-400 rounded-full absolute bottom-1"></div>' : ''}</div>`;
            }
        }
        function selectCalendarDay(iso) {
            selectedDayStr = iso; renderFullCalendar(); document.getElementById('day-detail').classList.remove('hidden'); document.getElementById('detail-date').innerText = iso.split('-').reverse().join('/');
            document.getElementById('detail-events').innerHTML = (bData.eventos[iso] || []).map((e, i) => `<div class="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm text-xs"><span class="font-bold text-slate-700">${e.hora} - ${e.titulo}</span><button onclick="borrarEv('${iso}',${i})" class="text-red-300"><i class="fa-solid fa-trash"></i></button></div>`).join('') || '<p class="opacity-40 italic text-xs text-center">Día libre</p>';
        }
        function renderVaccines() {
            const nac = new Date(bData.fecha); const hoy = new Date(); const meses = (hoy.getFullYear()-nac.getFullYear())*12 + (hoy.getMonth()-nac.getMonth());
            document.getElementById('vaccine-suggestions').innerHTML = VACUNAS_2026.filter(v => v.m >= meses - 1 && v.m <= meses + 2).map(v => `<button onclick="addVaccineToCalendar('${v.n}', ${v.m})" class="bg-white text-indigo-600 text-[9px] px-3 py-2 rounded-full font-black border border-indigo-200">En mes ${v.m}: ${v.n.split(',')[0]}...</button>`).join('') || '<p class="text-[9px] text-indigo-400">Sin vacunas próximas.</p>';
        }
        function addVaccineToCalendar(n, m) {
            const nac = new Date(bData.fecha); const f = new Date(nac.setMonth(nac.getMonth() + m)); const iso = f.toISOString().split('T')[0];
            if(!bData.eventos[iso]) bData.eventos[iso] = []; bData.eventos[iso].push({ titulo: "💉 Vacuna: " + n, hora: "10:00" });
            save(); alert("Añadida al " + iso.split('-').reverse().join('/')); showTab('calendario'); selectCalendarDay(iso);
        }
        ...
        function updateGrowth() { const p = document.getElementById('input-peso').value, t = document.getElementById('input-talla').value; if(p && t) { bData.crecimiento.push({fecha: new Date().toISOString().split('T')[0], p, m: t}); save(); renderHeader(); alert("Guardado"); } }
        ...
        function tomarMedicina(idx) { const m = bData.medicinas[idx], ahora = Date.now(); if(m.ultimaToma && (ahora - m.ultimaToma) < m.cada * 3600000) { if(!confirm("Aún no toca. ¿Registrar?")) return; } bData.medicinas[idx].ultimaToma = ahora; registrar("💊 " + m.nombre); save(); renderMedicina(); }
        function nuevoMedicamento() { const n = prompt("Nombre:"), c = prompt("Cada (h):", "8"); if(n&&c) { bData.medicinas.push({nombre:n, cada:parseInt(c), ultimaToma:null}); save(); renderMedicina(); } }
        ...
        function renderLog() { const logs = bData.logs[new Date().toISOString().split('T')[0]] || []; document.getElementById('log-recent').innerHTML = logs.slice(0,5).map(l => `<div class="bg-white p-3 rounded-xl flex justify-between text-xs shadow-sm border-l-4 border-blue-400"><span>${l.txt}</span><span class="text-slate-300">${l.h}</span></div>`).join(''); }
        ...
        function saveCustomEvent() { const t = document.getElementById('ev-titulo').value, h = document.getElementById('ev-hora').value; if(!t || !h) return; if(!bData.eventos[selectedDayStr]) bData.eventos[selectedDayStr] = []; bData.eventos[selectedDayStr].push({ titulo: t, hora: h }); save(); closeEventModal(); selectCalendarDay(selectedDayStr); }
        ...
        function finalizarSetup() { const p = document.getElementById('setup-peso').value, m = document.getElementById('setup-medida').value; bData = { nombre: document.getElementById('setup-nombre').value, fecha: document.getElementById('setup-fecha').value, logs: {}, crecimiento: [{fecha: new Date().toISOString().split('T')[0], p, m}], medicinas: [], eventos: {}, compra: [], intervaloToma: 3 }; save(); location.reload(); }
