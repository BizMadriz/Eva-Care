...
<body class="bg-slate-50 text-slate-900 pb-28">
    ...
    <div id="setup-wizard" class="fixed inset-0 bg-white z-[1000] flex flex-col p-8 hidden">
        <div id="step-1" class="flex flex-col h-full justify-center space-y-6">
            <h2 class="text-4xl font-extrabold tracking-tighter text-slate-900">EvaCare</h2>
            ...
            <button onclick="validaPaso(1, 2)" class="bg-blue-600 text-white p-6 rounded-3xl font-black">CONTINUAR</button>
        </div>
        <div id="step-2" class="hidden flex flex-col h-full justify-center space-y-6">
            ...
            <button onclick="validaPaso(2, 3)" class="bg-blue-600 text-white p-6 rounded-3xl font-black">SIGUIENTE</button>
        </div>
        <div id="step-3" class="hidden flex flex-col h-full justify-center space-y-6">
            ...
            <button onclick="finishSetup()" class="bg-slate-900 text-white p-6 rounded-3xl font-black">EMPEZAR</button>
        </div>
    </div>
    ...
    <main id="tab-inicio" class="p-4 space-y-4">
        ...
        <header class="pt-4 pb-2">
            ...
            <div class="bg-blue-50/50 p-5 rounded-[2rem] border border-blue-100/50">
        ...
        <div id="card-compra" class="card border-l-4 border-orange-400 hidden bg-orange-50/30">
            <h3 class="text-[10px] font-black text-orange-600 uppercase mb-2 tracking-widest">Pendiente de Comprar:</h3>
            <div id="lista-compra" class="space-y-2"></div>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <button onclick="openModal('modal-pecho')" class="bg-white border border-slate-100 h-24 rounded-[2rem] flex flex-col items-center justify-center gap-2 shadow-sm">
            ...
            <button onclick="openModal('modal-bibe')" class="bg-white border border-slate-100 h-24 rounded-[2rem] flex flex-col items-center justify-center gap-2 shadow-sm">
    ...
    <main id="tab-cal" class="hidden p-4 space-y-4">
        <div class="card">
            <h3 id="cal-mes" class="font-black text-slate-900 uppercase text-center mb-4"></h3>
    ...
    <main id="tab-salud" class="hidden p-4 space-y-4">
        <div class="card">
            <h3 class="text-xs font-black text-blue-600 uppercase mb-4">Añadir Tratamiento</h3>
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <input id="m-h" type="number" placeholder="Cada horas" class="p-4 bg-slate-50 rounded-2xl font-bold border-none outline-none">
                    <input id="m-d" type="number" placeholder="Días duración" class="p-4 bg-slate-50 rounded-2xl font-bold border-none outline-none">
                </div>
                <button onclick="addMed()" class="w-full bg-blue-600 text-white p-4 rounded-2xl font-black shadow-lg">GUARDAR</button>
            </div>
        </div>
    ...
    <div id="modal-menu" class="hidden fixed inset-0 bg-black/80 z-[1000] flex justify-end">
        <div class="bg-white w-4/5 h-full p-8 space-y-8 overflow-y-auto">
            <h2 class="text-3xl font-extrabold italic">Ajustes</h2>
            <div class="space-y-4">
                <div class="bg-slate-50 p-6 rounded-3xl">
                    <p class="text-[10px] font-black text-blue-600 uppercase mb-2 tracking-widest">Aviso tomas (h)</p>
                ...
                <div class="pt-8">
                    <button onclick="reiniciarApp()" class="w-full p-4 bg-red-50 text-red-600 rounded-2xl font-bold text-xs">REINICIAR APP</button>
                </div>
            </div>
            <button onclick="closeModal('modal-menu')" class="w-full text-slate-300 font-bold uppercase text-[10px]">Cerrar</button>
        </div>
    </div>
    ...
    <div id="modal-cal" class="hidden fixed inset-0 bg-black/80 z-[1000] flex items-center justify-center p-6">
        <div class="bg-white w-full rounded-[2.5rem] p-8 space-y-4">
            ...
            <input id="cal-ev-txt" type="text" placeholder="Evento" class="w-full p-4 bg-slate-100 rounded-2xl font-bold outline-none">
            <input id="cal-ev-hora" type="time" class="w-full p-4 bg-slate-100 rounded-2xl font-bold text-center">
            <button onclick="saveCita()" class="w-full bg-slate-900 text-white p-5 rounded-2xl font-black">GUARDAR</button>
    ...
    <script>
        // CAMBIO DE VERSIÓN PARA ASEGURAR LIMPIEZA
        let DATA = JSON.parse(localStorage.getItem('EVA_CARE_V9')) || {
        ...
        function save() {
            localStorage.setItem('EVA_CARE_V9', JSON.stringify(DATA));
            render();
        }

        function validaPaso(actual, sig) {
            const val = document.getElementById(`setup-${actual === 1 ? 'n' : 'f'}`).value;
            if(!val) return alert("Completa el campo");
        ...
        function render() {
            ...
            document.getElementById('lista-salud').innerHTML = be.meds.map((m, i) => `
                <div class="card flex justify-between items-center border-l-4 border-indigo-500 mb-2 relative">
                    <button onclick="deleteMed(${i})" class="absolute top-2 right-4 text-slate-200 text-xs">BORRAR</button>
                    <div><b class="text-indigo-600 text-xs uppercase">${m.n}</b><br><span class="text-[9px] text-slate-400">Días: ${m.d || '-'} | Cada ${m.h}h</span></div>
        ...
        function renderTimeline() {
            ...
            if(ultToma) {
                ...
                <div class="flex justify-between items-center timeline-item">
                    <div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 text-xs"><i class="fa-solid fa-baby"></i></div>
                    <div><p class="text-[10px] font-black uppercase text-slate-400">Siguiente Toma</p><p class="text-[9px] text-slate-500 italic">Última: ${ultToma.hora}</p></div></div>
            ...
            be.meds.forEach(m => {
                if(m.last > 0) {
                    ...
                    <div class="flex justify-between items-center pt-2 border-t border-white/5 timeline-item">
                        <div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center text-indigo-400 text-xs"><i class="fa-solid fa-pills"></i></div>
                        <p class="text-[10px] font-black uppercase text-slate-300">${m.n}</p></div>
            ...
            citasHoy.forEach(cita => {
                ...
                html += `
                <div class="flex justify-between items-center pt-2 border-t border-white/5 timeline-item bg-blue-500/5 p-2 rounded-xl">
                    <div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-orange-500/20 flex items-center justify-center text-orange-400 text-xs"><i class="fa-solid fa-calendar"></i></div>
                    <p class="text-[10px] font-black uppercase text-orange-400">${cita.txt}</p></div>
                    <span class="text-sm font-black ${restCita < 0 ? 'opacity-30' : 'text-white'}">${formatTime(restCita)}</span>
                </div>`;
            });

            container.innerHTML = html || "<p class='text-center text-[10px] text-slate-600 uppercase tracking-widest py-4'>Agenda libre</p>";
        }
        ...
        function addMed() {
            const n = document.getElementById('m-n').value, h = document.getElementById('m-h').value, d = document.getElementById('m-d').value;
            if(!n || !h) return alert("Mínimo Nombre y Horas");
            b().meds.push({n, h: parseInt(h), d: d, last: 0}); save();
        }
        function darMed(i) {
            const m = b().meds[i];
            if(m.last > 0 && (Date.now() - m.last) < (m.h * 3600000)) {
                if(!confirm("Aún no toca. ¿Dar dosis igualmente?")) return;
            }
            m.last = Date.now(); addLog(`💊 Dosis: ${m.n}`);
        }
        ...
        function renderCal() {
            ...
            for(let i=1; i<=total; i++) {
                ...
                grid.innerHTML += `<div onclick="openDay('${dStr}')" class="cal-day ${i==hoy.getDate()?'bg-blue-600 text-white':''}">${i}${tiene?'<div class="event-dot"></div>':''}</div>`;
            }
        }
