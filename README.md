...
<body class="text-slate-900 pb-28">
    ...
    <div id="modal-add-med" class="hidden fixed inset-0 bg-black/80 z-[6000] flex items-center justify-center p-6">
        <div class="bg-white w-full rounded-[2.5rem] p-8 space-y-4">
            <h3 class="text-xs font-black text-indigo-600 text-center uppercase">Nueva Medicina</h3>
    ...
    <div id="modal-add-crecimiento" class="hidden fixed inset-0 bg-black/80 z-[6000] flex items-center justify-center p-6">
        <div class="bg-white w-full rounded-[2.5rem] p-8 space-y-4">
            <h3 class="text-xs font-black text-emerald-600 text-center uppercase">Actualizar Medidas</h3>
            <div class="grid grid-cols-2 gap-2">
            ...
            <button onclick="updateCrecimiento()" class="w-full bg-emerald-600 text-white p-5 rounded-2xl font-black uppercase text-xs">Actualizar</button>
    ...
    <div id="modal-menu" class="hidden fixed inset-0 bg-black/80 z-[6000] flex justify-end">
        <div class="bg-white w-4/5 h-full p-8 flex flex-col">
            ...
            <div class="flex-1 space-y-4">
                ...
                <button onclick="reiniciarApp()" class="w-full p-4 bg-red-50 text-red-600 rounded-2xl font-bold text-[10px] uppercase mt-10">Borrar todo</button>
            </div>
    ...
    <script>
        let DATA = JSON.parse(localStorage.getItem('EVA_CARE_V15')) || { idx: 0, bebes: [], lastBibe: 0, plazo: 3 };
        ...
        function darMed(i) { b().meds[i].last = Date.now(); addLog(`💊 ${b().meds[i].n}`); save(); }
        function updateCrecimiento() {
            const p = document.getElementById('up-p').value, a = document.getElementById('up-a').value;
            if(p) b().curP = p; if(a) b().curA = a;
            closeModal('modal-add-crecimiento'); save();
        }
        ...
        function renderCal() {
            ...
            for (let i = 1; i <= ultimoDiaMes; i++) {
                ...
                const tieneEvento = b().citas[dStr] && b().citas[dStr].length > 0;
                const esHoy = i === new Date().getDate() && month === new Date().getMonth();
                grid.innerHTML += `<div onclick="openDay('${dStr}')" class="cal-day ${esHoy ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-slate-500'}">${i}${tieneEvento ? '<div class="event-dot"></div>' : ''}</div>`;
            }
        ...
        function renderAgendaCompleta() {
            ...
            const citas = [];
            Object.keys(b().citas).forEach(fecha => b().citas[fecha].forEach((c, idx) => citas.push({...c, fecha, idx})));
            citas.sort((a,b) => new Date(a.fecha+'T'+a.hora) - new Date(b.fecha+'T'+b.hora));
            container.innerHTML = citas.map(c => `
                <div class="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border border-slate-50 mb-2">
                    <div class="flex flex-col"><span class="text-[8px] font-black text-blue-500 uppercase">${new Date(c.fecha).toLocaleDateString('es', { day: 'numeric', month: 'short' })}</span><span class="text-xs font-bold text-slate-700">${c.txt}</span></div>
                    <div class="flex items-center gap-3"><span class="text-[9px] font-bold text-slate-400">${c.hora}</span><button onclick="deleteCita('${c.fecha}', ${c.idx})" class="text-red-300"><i class="fa-solid fa-trash text-[10px]"></i></button></div>
                </div>`).join('');
        }
        function saveCita() {
            ...
            closeModal('modal-cal'); save();
        }
        function deleteCita(f, i) { b().citas[f].splice(i, 1); if(b().citas[f].length === 0) delete b().citas[f]; save(); }
        ...
        function showTab(id) {
            ['inicio', 'cal', 'salud'].forEach(t => {
                ...
                document.getElementById('nav-' + t).classList.toggle('nav-active', t === id);
            });
            window.scrollTo(0,0);
        }
        ...
        render();
        if(DATA.bebes.length > 0) showTab('inicio');
    </script>
</body>
</html>
